...
(module engine racket/gui
          ...
          (hash-update! score-table killer
                        (λ (scorelist)
                          (list*
                           ...
                           (cadr scorelist)
                           (+ (caddr scorelist)
                              (if (eq? killer victim) 0
                                  (hash-ref points-table (cons killer-tag victim-tag) 0)))
                           (if (and (is-a victim 'generator?)
                                    (not (eq? killer victim)))
                               (+ (cadddr scorelist) 1)
                               (cadddr scorelist))
                           (cddddr scorelist)))
                        '(0 0 0 0))
  ...
  (define (find-winners [min-winners 2])
    (let* ((table (hash-map score-table cons))
      ...
      (displayln "Cumulative Scores:")
      (displayln (hash-map full-score-table (λ (key val)
                                         (cons (ask key 'name)
                                               (append (obj-stats key)
                                                       val)))))
      (list (map (λ (pl)
                   (cons (ask (car pl) 'name)
                         (cdr pl)))
                 sorted-table)
            (map (λ (pl)
                   (append (list (ask pl 'name)) (obj-stats pl) (hash-ref full-score-table pl)))
                 (with-handlers ((exn:fail:contract? (const sorted-table)))
                   (let ((cut-off (list-ref sorted-table (- min-winners 1))))
                     (filter-map
                      (λ (x)
                        (if (scoreboard-comp x cut-off)
                            (car x)
                            #f))
                      sorted-table)))))))
  
  (define scoreboard-comp
    (λ (a b)
      (let ((ptsA (cadddr a))
            (ptsB (cadddr b))
            (bombsA (car (cddddr a)))
