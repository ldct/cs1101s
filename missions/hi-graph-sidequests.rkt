...
;;; Update 17/08/10: Defined new drawing primitive draw-connected-full-view-proportional and function full-view-proportional
;;;
...
(define (joe-rotate theta) ;rotates around origin, but less efficiently
...
;;; FULL-VIEW 
(define (full-view-proportional xlo xhi ylo yhi)
  (let ((width (- xhi xlo))
        (height (- yhi ylo)))
    ...
    (if (or (zero? width) (zero? height))
        ...
        (let* ((scale-factor (min (* 0.9 (/ 1 width)) (* 0.9 (/ 1 height))))
               (new-mid-x (* scale-factor (/ (+ xlo xhi) 2)))
               (new-mid-y (* scale-factor (/ (+ ylo yhi) 2))))
          (compose (translate (- 0.5 new-mid-x) (- 0.5 new-mid-y)) (scale-x-y scale-factor scale-factor))))))
...
(define (connect-ends curve1 curve2)
  (let* ((start-point-of-curve2 (curve2 0))
         (end-point-of-curve1 (curve1 1)))
    (connect-rigidly curve1 
                     ((translate (- (x-of end-point-of-curve1)
                                    (x-of start-point-of-curve2))
                                 (- (y-of end-point-of-curve1)
                                    (y-of start-point-of-curve2)))
                      curve2))))
...
(define (zc-dragonize n curve)
  (if (= n 0)
      curve
      (let ((c (zc-dragonize (- n 1) curve)))
        (put-in-standard-position
         (connect-ends
          ((rotate-around-origin (- (/ pi 2))) c)
          c)))))
...
;; makes the curve fully visible and centralized in viewport, maintaining original proportions
(define (draw-connected-full-view-proportional window n)
  (lambda (curve)
    ((draw-connected window n)
    (((corners curve n) full-view-proportional) curve))))
