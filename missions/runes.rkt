;;; Quilting/stereogram support code for CS1101S Problem Set 1

;;; Authors:
;;; v1 (2008/09) by Caleb Chao & Benny Ou -- August 2008
;;; v2 (2009/10) by Chris Henry -- June 2009
;;; v3 (2010/11) by Caleb Chao -- August 2010
;;; v3.1 (2011/12) by He Junwei -- August 2011

;;; v3.1 changelog:
;;; - Renamed round to round-off as it is a predefined procedure

;;; v3 changelog:
;;; - Updated from scheme to racket - vector-fill! updates vector instead of returning
;;; - Added anaglyph to display Red-Cyan anaglyphs
;;; - Added invisible pixmaps lp and rp for anaglyph information
;;; - Editted all painters to update lp and rp
;;; - Editted painter generating procedures to update lp and rp
;;; - Reworked anaglyph and show for faster performance
;;; - Reworked image->painter for smoother finish
;;; - Implemented hollusion (compliments to Koh Zi Chun)
;;; - Reworked transformation translate to take ratios
;;; - Added save-custom-animation taking list of buffered frames and list of frame durations

;;; v2 changelog:
;;; - Rewrote circle-bb and added lots of comments.
;;; - Remove vp argument for show, stereogram, and clear.
;;; - Renamed test-bb to sail-bb.
;;; - Improved the code style.
;;; - Added transformation translate.
;;; - Added scale-independent into process-frame. This scales x and y coords separately.
;;; - Removed the original scale from process-frame; rewrote scale using scale-independent.
;;; - Slight improvement to heart-bb (extracted repeated process using let;
;;;   improve readability by extracting calculation to let definition).
;;; - Rename white-bb to blank-bb.
;;; - blank-bb now returns "(void)" to avoid printing of ().
;;; - Rewrote spiral-bb, eliminated repeated codes.
;;; - Completely rewrote function->pointer:
;;;   * Depth function takes input in the range [0, 100];
;;;   * Sample the depth function at integer intervals;
;;;   * Viewport is sampled and drawn uniformly (works well with scaling, translation, etc.);
;;;   * Uses more natural looping construct using for-each and integer sequence/list.
;;; - Reimplemented overlay in terms of overlay-frac.
;;; - Removed 'deep and 'shallow commands from transformation factory (since we reimplement
;;;   overlay in terms of overlay-frac, we don't need them anymore).
;;; - Similarly reimplemented stack in terms of stack-frac (and removed the corresponding
;;;   'top and 'bottom commands from transformation factory).
;;; - Wrote 'rotate transformation (in the factory); removed 'rotate45 (which is broken).
;;; - Rewritten the broken eighth-turn-left in terms of 'rotate.

;;; v2 bugs (low/no priority):
;;; - 'rotate when applied to quarter-turn-right screws heart-bb. Current fix:
;;;   all transformation except eighth-turn-left uses the original 'rotate90 code.
...
(define lp (open-pixmap "Left Port" (* 4/3 viewport-size) viewport-size))
(define rp (open-pixmap "Right Port" (* 4/3 viewport-size) viewport-size))
(define (clear) (begin
...
; Red-Blue anaglyph generation function
